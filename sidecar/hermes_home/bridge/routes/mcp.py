...
from ..validation import safe_mcp_url
...
class CreateMcp(BaseModel):
    ...
    # Bound free-text fields so huge payloads can't be stuffed into the store.
...
async def add(body: CreateMcp) -> dict:
    # Reject ``file://``, raw IPs in private ranges, and cloud metadata hosts
    # before they reach the store and later get dialled by the agent.
    validated_url = safe_mcp_url(body.url)
    store = get_store()
